...
<div v-if="!submitted">
	...
	<tr>
	<th>아이디용이름 체크</th>
	<td><input data-msg="이름"  type="text" name="username" id="_username2"  
	v-model="customer.username" @click="checkusername" style="width:30%"/></td>
	</tr>
	</table>
	</div>
...
<tr>
<th>아이디용이름</th>
<td><input data-msg="이름"  type="text" name="username" id="_username"  
v-model="customer.username" @click="checkusername" style="width:30%"/></td>
</tr>
<tr>
<th>이름</th>
...
export default {
	...
	methods: {
		checkusername(){
				...
				http3.get('users/'+this.customer.username).then(response => {
				//console.log(response.data);
				if (response.data.id) {
					alert("이미 존재하는 아이디네입니다.");
					this.newCustomer();
					document.getElementById("_username").attr("readonly","");
				}else{
					alert("사용할 수 있는 아이디네입니다.");
					document.getElementById("_username").attr("readonly","readonly");
				}
			}).catch(() => {
					alert("가입을 하지 못했습니다.");
